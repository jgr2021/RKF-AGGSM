%% AGSlash 噪声仿真（高斯 + Slash）– 全滤波器对比 + 等效 CRLB
% 包含 RKF‑Slash 和运行时间统计
clear; clc; close all;
rng(2025);

%% ================== 选择场景（1~4） ==================
case_id = 4;   % 1:Mild, 2:Moderate, 3:Severe, 4:Extreme
...
case_data = AGSlash_cases{case_id};
...
%% ================== CRLB（基于等效高斯近似） ==================
% 计算总方差（高阶矩存在需 alpha_s > 2，否则近似）
if alpha_s > 2
    var_slash = scale_s * (alpha_s / (alpha_s - 2));  % Slash 分布方差（当 alpha>2）
else
    var_slash = 100;  % 重尾时放大
end
R_total = R_g + var_slash;
CRLB_vals = compute_CRLB_Gaussian(F, H, Q, R_total, N_steps, eye(n));
fprintf('等效高斯 CRLB 计算完成（基于总方差）\n');

%% ================== 加载滤波器超参数 ==================
% 注意：这里需要 Huber, GSTM, TAU 的超参数，可以使用 AGST 的调优结果或默认值。
% 为了简化，直接使用与 AGST 相同的默认值（或从文件加载）。
...
% AGGSM 预设参数（使用与 AGST 相同的调优文件，或默认）
...
% 加载 RKF‑Slash 最佳参数（针对 AGSlash 场景）
slash_params_exist = false;
if exist('AGSlash_best_params_AGSlash.mat', 'file')
    load('AGSlash_best_params_AGSlash.mat', 'best_params');
...
if ~slash_params_exist
    ...
    warning('未找到 AGSlash_best_params_AGSlash.mat，使用与 AGSMG 相同的默认参数。');
end
...
%% ================== 结果保存与绘图 ==================
save(sprintf('AGSlash_results_Case%d.mat', case_id), ...
    ...
    'CRLB_vals', 'R_g','alpha_s','scale_s','R_Gaussian','R_St','dof_St',...
...
fprintf('结果已保存至 AGSlash_results_Case%d.mat\n', case_id);